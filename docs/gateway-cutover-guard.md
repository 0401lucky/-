...
- 农场：已允许当前前端使用路径和旧商店只读兼容入口共 20 条精确路径，仍禁止 `/api/farm*` 通配和路径改写。
...
- 商城/钱包：已允许 `/api/store`、`/api/store/exchange`、`/api/store/topup`、`/api/store/withdraw`、`/api/store/admin` 五条精确路径；仍禁止 `/api/store*` 通配。
- 卡牌前台/后台：已允许精确路径，`/api/cards/purchase` 只返回 410 墓碑响应；仍禁止根路径和通配，避免误转发未审路径。
- `/api/games/overview`：已精确切到 Go，用于收口旧 Next/KV 回落风险。
- `/api/games/*`：普通游戏和游戏汇总已用精确路径切流，仍禁止通配。
