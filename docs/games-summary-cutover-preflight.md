...
当前结论：`/api/games/profile` 与 `/api/games/overview` 均已完成精确切流，并通过直连 Go API 聚合冒烟；`/api/games/overview` 额外覆盖经 Gateway 的认证冒烟。完整 `/api/games/*` 通配仍保持关闭。
...
`GET /api/games/overview` 旧 Next 路由仍存在，但 Gateway 已精确转发到 Go，用于收口 Next/KV 回落风险。
...
- 认证 `GET /api/games/overview` 经 Gateway 返回兼容字段。
- Gateway 只允许已切流的 `handle /api/games/profile {` 与 `handle /api/games/overview {`，不允许 `/api/games/*`。
- 最后自动清理测试用户、积分账户、今日统计和游戏记录。

## 精确 Gateway 草案

当前已启用已审精确规则，不打开 `/api/games/*` 通配：
...
7. Gateway 只允许精确打开 `/api/games/profile` 与 `/api/games/overview`；不要打开 `/api/games/*`。
...
1. 从 `gateway/Caddyfile` 移除 `handle /api/games/profile` 与 `handle /api/games/overview`。
2. 重建并重启 `gateway`。
3. 复验 `/api/games/profile` 与 `/api/games/overview` 回落到 Next。
