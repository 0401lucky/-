...
export default function Game2048Page() {
  ...
  const handleSettlementSuccess = useCallback((
    ...
    setPhase('finished');
    setMessage(fallback ? `已启用兜底结算，本局获得 ${record.pointsEarned} 积分` : `本局获得 ${record.pointsEarned} 积分`);
    void fetchStatus();
  ...
  const settleGame = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !data?.success || !data.data) {
        if (res.status >= 500) {
          const fallback = await requestGameFallback<Game2048Record>({
            game: '2048',
            ...payload,
          });
          if (fallback) {
      ...
      handleSettlementSuccess(data.data, false);
    } catch (err) {
      try {
        const fallback = await requestGameFallback<Game2048Record>({
          game: '2048',
        ...
        console.error('2048 fallback settlement error:', fallbackError);
      }
