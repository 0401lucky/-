    ...
    <div className="memory-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="memory-settlement-title">
      <div className={`memory-result-modal ${won ? 'won' : 'lost'}`}>
        <div className="flex flex-col items-center text-center">
          ...
          <h2 id="memory-settlement-title" className="mt-1 text-2xl font-black text-slate-950">
            {won ? '胜利结算完成' : '失败结算完成'}
          </h2>
        ...
        <div className="memory-result-stats">
          ...
          <MemoryResultStat label="用时" value={formatDuration(duration)} />
          <MemoryResultStat label="步数" value={`${moves} 步`} />
          <MemoryResultStat label="得分" value={String(score)} />
        </div>
