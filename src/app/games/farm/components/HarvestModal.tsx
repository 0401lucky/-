...
import type { HarvestResult, LevelUpInfo, BatchHarvestResult } from '../hooks/useFarmState';

interface HarvestModalProps {
  result: HarvestResult | null;
  batchResult: BatchHarvestResult | null;
  levelUp: LevelUpInfo | null;
  onClose: () => void;
}

export default function HarvestModal({ result, batchResult, levelUp, onClose }: HarvestModalProps) {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
        ...
        >
          ...
          <div className="bg-gradient-to-r from-amber-400 to-orange-500 text-white px-6 py-6 text-center relative overflow-hidden">
            ...
            <span className="text-5xl block mb-2 relative z-10 drop-shadow-lg animate-farm-harvest">🎉</span>
            <h3 className="text-xl font-bold relative z-10">批量收获成功！</h3>
            <p className="text-amber-100 text-sm mt-1 relative z-10">共收获 {batchResult.harvestedCount} 块田地</p>
          </div>

          {/* 收获列表 */}
          <div className="p-6 space-y-3">
            {batchResult.harvests.length > 0 && (
              <div className="space-y-2 max-h-40 overflow-y-auto">
                {batchResult.harvests.map((h, i) => (
                  <div key={i} className="flex items-center justify-between text-sm bg-slate-50/80 rounded-lg px-3 py-2">
                    <span className="flex items-center gap-1.5">
                      <span>{h.cropIcon}</span>
                      <span className="text-slate-700">{h.cropName}</span>
                    </span>
                    <span className="font-semibold text-amber-600">+{h.finalYield}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="border-t border-slate-100 pt-3 flex items-center justify-between">
              <span className="text-slate-700 font-semibold">总积分</span>
              <span className="text-xl font-bold text-amber-600 drop-shadow-sm">+{batchResult.totalPointsEarned} ⭐</span>
            </div>

            <div className="flex items-center justify-between text-sm">
              <span className="text-slate-500">总经验</span>
              <span className="font-semibold text-green-600">+{batchResult.expGained} EXP</span>
            </div>
