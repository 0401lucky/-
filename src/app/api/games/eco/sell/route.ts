...
import { sellEcoPrize } from '@/lib/eco';
...
export const POST = withUserRateLimit('store:exchange', async (request: NextRequest, user) => {
  ...
  try {
    body = (await request.json()) as { key?: unknown; quantity?: unknown };
  } catch {
  ...
  const result = await sellEcoPrize(user.id, body.key as EcoPrizeKey, quantity);
  if (!result.ok) {
