...
import { harvestPlot, getFarmStatus } from '@/lib/farm-v2';
...
export const POST = withUserRateLimit(
  ...
  async (req: NextRequest, user) => {
    try {
      ...
      const r = await harvestPlot(user.id, body.plotIndex);
      if (!r.ok) return NextResponse.json({ success: false, message: r.msg }, { status: 400 });
      const data = await getFarmStatus(user.id);
      return NextResponse.json({ success: true, data, harvest: r.result, balance: r.balance });
    } catch (e) {
      console.error('farm v2 harvest error:', e);
