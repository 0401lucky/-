...
import { waterPlot, getFarmStatus } from '@/lib/farm-v2';
...
export const POST = withUserRateLimit(
  ...
  async (req: NextRequest, user) => {
    try {
      ...
      const r = await waterPlot(user.id, body.plotIndex);
      if (!r.ok) return NextResponse.json({ success: false, message: r.msg }, { status: 400 });
      const data = await getFarmStatus(user.id);
      return NextResponse.json({ success: true, data, bonus: r.bonus ?? 0 });
    } catch (e) {
      console.error('farm v2 water error:', e);
