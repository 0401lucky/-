...
import { getEcoTrashLeaderboard, type EcoTrashRankingPeriod } from '@/lib/eco';
...
function normalizePeriod(value: string | null): EcoTrashRankingPeriod {
...
export const GET = withAuthenticatedUser(
  async (request) => {
    try {
      ...
      const data = await getEcoTrashLeaderboard(period, limit);
      ...
      if (kvInsight.isUnavailable) {
        return NextResponse.json(
          buildKvUnavailablePayload('环保排行榜服务暂时不可用，请稍后重试'),
          {
            status: 503,
            headers: {
      ...
      console.error('Get eco rankings error:', error);
      return NextResponse.json(
        { success: false, message: '获取环保排行榜失败' },
        { status: 500 },
      );
    }
  },
