...
import { formatChinaDateTime, formatChinaDateTimeInput, parseChinaDateTimeInput } from '@/lib/time';
...
export default function AdminRaffleDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSaveEdit = async () => {
    ...
    if (editMode === 'draw') {
      ...
      if (editTriggerType === 'scheduled' && parseChinaDateTimeInput(editScheduledDrawAt) === null) {
        ...
        body: JSON.stringify({
          ...
          ...(editMode === 'draw'
            ? {
                ...
                scheduledDrawAt: editTriggerType === 'scheduled'
                  ? parseChinaDateTimeInput(editScheduledDrawAt) ?? undefined
                  : undefined,
