...
export default function UsersPage() {
  ...
  const handleSyncUsers = async () => {
    ...
    try {
      ...
      if (data.success) {
        await fetchUsers();
        window.alert(data.message || '同步完成');
      } else {
        window.alert(data.message || '同步失败');
      }
    } catch (error) {
  ...
  const handleMigrateNewUserEligibility = async () => {
    ...
    const confirmed = window.confirm(
      '将把历史上已领取过新人项目的用户标记为已使用新人资格。是否继续？',
    );
    if (!confirmed) return;
    ...
    try {
      ...
      if (data.success) {
        await fetchUsers();
        window.alert(data.message || '迁移完成');
      } else {
        window.alert(data.message || '迁移失败');
      }
    } catch (error) {
      console.error('Migrate eligibility error:', error);
      window.alert('迁移失败');
    } finally {
      ...
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="inline-flex h-10 items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-4 text-sm font-semibold text-amber-700 transition hover:bg-amber-100 disabled:cursor-not-allowed disabled:opacity-60"
          >
            <Star className={`h-4 w-4 ${migratingEligibility ? 'animate-pulse' : ''}`} />
          ...
          <button
            ...
            disabled={syncing}
            className="inline-flex h-10 items-center gap-2 rounded-lg border border-stone-200 bg-white px-4 text-sm font-semibold text-stone-700 transition hover:bg-stone-50 disabled:cursor-not-allowed disabled:opacity-60"
          >
          ...
          <div className="overflow-hidden rounded-lg border border-stone-200 bg-white">
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <p className="mt-1 text-xs text-stone-400">调整搜索条件或同步历史用户后再查看。</p>
              </div>
            ) : (
