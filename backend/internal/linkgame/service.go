...
var ErrUnavailable = errors.New("linkgame database unavailable")
...
func (service *Service) Start(ctx context.Context, user auth.User, input StartInput) (StartResult, error) {
	...
	err := service.withRetryableTx(ctx, func(tx pgx.Tx) error {
		...
		session := Session{
			...
			Seed:       seed,
			TileLayout: GenerateTileLayout(difficulty, seed),
			StartedAt:  nowMs,
...
func (service *Service) Submit(ctx context.Context, user auth.User, input SubmitInput) (SubmitResult, error) {
	...
	if input.SessionID == "" || input.Moves == nil {
	...
	err := service.withRetryableTx(ctx, func(tx pgx.Tx) error {
		...
		validation := ValidateResult(*session, input)
		if !validation.OK {
			output = SubmitResult{Success: false, Message: validation.Message}
			return nil
		}
		serverDuration := nowMs - session.StartedAt
		config := DifficultyConfigFor(session.Difficulty)
		if ok, message := ValidateSettlementTiming(serverDuration, config, validation.Outcome); !ok {
			output = SubmitResult{Success: false, Message: message}
			return nil
		}

		score := int64(0)
		if validation.Completed || validation.Outcome == OutcomeDeadlock || (session.Difficulty == DifficultyHard && validation.Outcome == OutcomeTimeout) {
			combo := maxInt(0, validation.MaxStreak-1)
			if session.Difficulty == DifficultyHard {
				combo = 0
			}
			timeRemainingSeconds := maxInt64(0, config.TimeLimit-serverDuration/1000)
			score = CalculateScore(validation.MatchedPairs, config.BaseScore, combo, timeRemainingSeconds, session.Difficulty, config.Pairs, validation.Outcome)
		}
		pointReward := CalculatePointReward(score, session.Difficulty, validation.Outcome)
		dailyLimit, err := systemconfig.DailyPointsLimit(ctx, tx)
		if err != nil {
			return err
		}
		pointsEarned, dailyEarned, err := addGamePointsWithLimit(
			...
			pointReward,
			dailyLimit,
			fmt.Sprintf("连连看%s得分 %d，福利积分 %d", outcomeLabel(validation.Outcome), score, pointReward),
		)
		if err != nil {
			return err
		}
		record := Record{
			...
			Moves:            len(input.Moves),
			Completed:        validation.Completed,
			Outcome:          validation.Outcome,
			SettlementResult: GetSettlementResult(validation.Completed, validation.Outcome),
			Score:            score,
...
func outcomeLabel(outcome SettlementOutcome) string {
