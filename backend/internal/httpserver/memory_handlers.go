...
func newMemoryHandlers(deps Dependencies) memoryHandlers {
	return memoryHandlers{
		deps:    deps,
		service: memory.NewService(deps.DB),
	}
}

func (handlers memoryHandlers) status(writer http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		handlers.writeServiceError(writer, "查询记忆游戏状态失败", err)
		return
	}
	writeJSON(writer, http.StatusOK, map[string]any{"success": true, "data": data})
}

func (handlers memoryHandlers) start(writer http.ResponseWriter, request *http.Request) {
	shared := economyHandlers{deps: handlers.deps}
	...
	if _, ok := memory.DifficultyConfigs[payload.Difficulty]; !ok {
		writeJSON(writer, http.StatusBadRequest, map[string]any{"success": false, "message": "无效的难度选择"})
		return
	}
	result, err := handlers.service.Start(request.Context(), *user, payload.Difficulty)
	if err != nil {
		handlers.writeServiceError(writer, "开始记忆游戏失败", err)
		return
	}
	if !result.Success {
	...
	writeJSON(writer, http.StatusOK, map[string]any{
		"success": true,
		"data":    memory.BuildSessionView(*result.Session),
	})
}

func (handlers memoryHandlers) flip(writer http.ResponseWriter, request *http.Request) {
	shared := economyHandlers{deps: handlers.deps}
	if shared.rejectUntrustedUnsafeRequest(writer, request) {
		return
	}
	...
	if strings.TrimSpace(payload.SessionID) == "" || payload.CardIndex == nil {
		writeJSON(writer, http.StatusBadRequest, map[string]any{"success": false, "message": "参数错误"})
		return
	}
	result, err := handlers.service.Flip(request.Context(), *user, payload.SessionID, *payload.CardIndex)
	if err != nil {
		handlers.writeServiceError(writer, "记忆游戏翻牌失败", err)
		return
	}
	if !result.Success {
		writeJSON(writer, http.StatusBadRequest, map[string]any{"success": false, "message": result.Message})
...
func (handlers memoryHandlers) submit(writer http.ResponseWriter, request *http.Request) {
	shared := economyHandlers{deps: handlers.deps}
	if shared.rejectUntrustedUnsafeRequest(writer, request) {
		return
	}
	...
	var payload memory.SubmitInput
	if err := json.NewDecoder(request.Body).Decode(&payload); err != nil {
		writeJSON(writer, http.StatusBadRequest, map[string]any{"success": false, "message": "参数错误"})
		return
	}
	...
	if err != nil {
		handlers.writeServiceError(writer, "记忆游戏结算失败", err)
		return
	}
	if !result.Success {
		writeJSON(writer, http.StatusBadRequest, map[string]any{"success": false, "message": result.Message})
...
func (handlers memoryHandlers) cancel(writer http.ResponseWriter, request *http.Request) {
	shared := economyHandlers{deps: handlers.deps}
	if shared.rejectUntrustedUnsafeRequest(writer, request) {
		return
	}
	...
	if err != nil {
		handlers.writeServiceError(writer, "取消记忆游戏失败", err)
		return
	}
	if !result.Success {
		writeJSON(writer, http.StatusBadRequest, map[string]any{"success": false, "message": result.Message})
...
func (handlers memoryHandlers) writeServiceError(writer http.ResponseWriter, logMessage string, err error) {
